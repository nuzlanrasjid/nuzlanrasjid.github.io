...
Summarize your actual numbers here, for example:

- Soil moisture model: R² (CV) = ..., RMSECV = ..., RPD = ...
- Soil temperature model: R² (CV) = ..., RMSECV = ..., RPD = ...

Note which wavelength regions had the strongest regression coefficients,
and what that might suggest biologically or physically (e.g. water
absorption bands).

## Next steps

- Validate the model on an independent field dataset
- Compare PLSR against other regression methods (Random Forest, SVR)
- Test on a wider spectral range or different soil types
